...
The idea is to use the feature in order to:
+ understand the family status (married or maiden)
+ wheater or not is part of the crew
+ handle the NA for the `Age`

Regular expressions have been used in order to exract the `Name.family` and `Name.title`
...
The less frequent titles are pooled in the more common ones in order to avoid overfitting:
...
g <- ggplot(data = filter(full,!is.na(Embarked)), aes(Embarked,Ticket.number))
...
Since the high number of missing values for the `Cabin` feature we're going to remove it:
...
```{r}
full <- full %>% mutate(Fare=ifelse(is.na(Fare),mean(Fare,na.rm = TRUE),Fare))
```
...
```{r}
full %>% select(PassengerId,Age,Parch,SibSp,Sex) %>% filter(Parch > 0) %>% arrange(Age)
```
...
train <- train %>% select(AnySurvivor, Survived, Age.bin, Sex, Pclass, Fare.group)
```
...
confusionMatrix.list <- function(predictions, outcome, ...){
        ...
        names(cmList) <- names(predictions)
        cmList
}
```
...
```{r}
df <- prediction.train.test %>% select(firstLayer) %>% mutate(Survived = df$Survived)
...
model.rfSecondLayer$results
performance[c("Accuracy","AccuracyLower","AccuracyUpper","Sensitivity","Specificity","Balanced Accuracy"),]
```
...
## Training using the whole train set
...
# 
# # Train
# models.train <- train.list(Survived~., df, modelTags, trControl=ctrl, trace = FALSE)
# 
# # Predict
# prediction.train <- predict.list(models.train,df)
# 
# # Stacking
# df.prediction <- prediction.train %>% select(firstLayer) %>% mutate(Survived = df$Survived)
# model.rfSecondLayer <- train(Survived~.,data=df.prediction, method="rf", trControl = ctrl)
# prediction.train$rfSecondLayer <- predict(model.rfSecondLayer,df.prediction)
# 
# # Majority vote
# df.prediction <- prediction.train %>% select(modelTagsMaj)
